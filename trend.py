...
def get_trending_symbols(limit: int = 80) -> List[str]:
    """Return liquid pairs with a balanced attention score, not only extreme pumps."""
    payload = None
    last_error = None
    ...
    if payload is None:
        logger.error("get_trending_symbols failed: %s", last_error)
        return []
    ...
    for item in payload if isinstance(payload, list) else []:
        ...
        try:
            ...
            change_pct = abs(float(item.get("priceChangePercent", 0.0)))
        ...
        if quote_volume < MIN_QUOTE_VOLUME or last_price <= 0:
            continue
        # Avoid ranking obvious one-day anomalies above every healthy trend.
        ...
        rows.append((symbol, attention_score, quote_volume))

    rows.sort(key=lambda row: (row[1], row[2]), reverse=True)
    return [symbol for symbol, _, _ in rows[: max(1, int(limit))]]
