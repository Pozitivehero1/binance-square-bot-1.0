...
ENABLE_AI_POLISH = os.getenv("ENABLE_AI_POLISH", "1").strip().lower() in {"1", "true", "yes"}
AI_VARIANTS = int(os.getenv("AI_VARIANTS", "1"))
POST_MAX_CHARS = int(os.getenv("POST_MAX_CHARS", "1200"))
...
def _contains_required_content(text: str, levels: Dict[str, float]) -> bool:
    """Validate that AI generated a real trade signal, not a generic opinion."""
    if not text:
        return False
    ...
    if not any(x in upper for x in [
        "LONG", "SHORT", "BUY", "SELL", "ЛОНГ", "ШОРТ",
        "ПОКУПК", "ПРОДА", "БЫЧ", "МЕДВЕЖ"
    ]):
        return False

    if not any(x in upper for x in ["ENTRY", "ВХОД", "ЗОНА ВХОДА"]):
        return False
    if not any(x in upper for x in ["TP", "TARGET", "ЦЕЛ", "ТЕЙК"]):
        return False
    if not any(x in upper for x in ["SL", "STOP", "СТОП"]):
        return False

    bad = ["НЕТ ДАННЫХ", "НЕ МОГУ", "Я НЕ ЗНАЮ", "ГАРАНТИРОВАННАЯ ПРИБЫЛЬ"]
...
def _polish_with_ai(text: str, basic: str, levels: Dict[str, float], style: PostStyle, angle: SignalAngle) -> str:
    ...
    if not MISTRAL_API:
        logger.warning("Mistral API key missing; AI author unavailable")
        return text
...
def generate_post_draft(
    *,
    ...
    if ENABLE_AI_POLISH and MISTRAL_API:
        try:
            logger.info("Author Mode generating final post for %s", symbol)
            candidate = author_post(author_data)
            if validate(candidate, author_data):
                post = candidate
                author_mode_used = True
                logger.info("Author Mode validation passed; final writer selected")
        except Exception as exc:
            logger.exception("AUTHOR MODE FAILED")

    # FINAL AUTHOR MODE: if AI answer is readable but misses a cosmetic rule, keep it.
    # Numbers are protected upstream; do not kill human posts with rigid template checks.
    if post is None and ENABLE_AI_POLISH and MISTRAL_API:
        fallback = author_post(author_data)
        if fallback and len(fallback) > 200:
            post = fallback
        else:
            raise RuntimeError("Author Mode did not produce a valid publication")

    # Legacy renderer is kept only for installations without Mistral.
    if post is None:
        logger.warning("Using legacy writer fallback (MISTRAL disabled)")
        post = _render_style(
            style=style,
            angle=angle,
