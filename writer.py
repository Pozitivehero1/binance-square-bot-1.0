    ...
    *,
    ...
    if ENABLE_AI_POLISH and MISTRAL_API:
        try:
            ...
            logger.warning("Author mode failed: %s", exc)

    # Legacy template only as emergency fallback.
    if post is None and not (ENABLE_AI_POLISH and MISTRAL_API):
        logger.warning("Using legacy writer fallback")
        post = _render_style(
            style=style,
            angle=angle,
