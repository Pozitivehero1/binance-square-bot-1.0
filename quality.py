...
class PostQualityEvaluator:
    ...
    MAX_LENGTH = int(os.getenv("POST_MAX_CHARS", "900"))
    ...
    def validate(
        ...
        *,
        ...
        required_groups = (
            ("entry", ("вход", "entry")),
            ("targets", ("цели", "tp1", "цель 1", "targets")),
            ("stop", ("стоп-лосс", "стоп", "stop-loss", "stop")),
        ...
        if basic:
            ...
            if not 1 <= ticker_count <= 3:
                reasons.append(f"ticker mentions {ticker_count}, expected 1-3")
    ...
    def _structure(text: str) -> float:
        lowered = text.lower()
        score = 0.0
        score += 20.0 if len([part for part in text.split("\n\n") if part.strip()]) >= 4 else 8.0
        score += 30.0 if all(marker in lowered for marker in ("вход", "цели", "стоп-лосс", "r/r")) else 0.0
        score += 18.0 if any(marker in lowered for marker in ("отмена:", "отмена сценария", "без усреднения")) else 0.0
        score += 14.0 if any(marker in lowered for marker in ("триггер", "подтверждение", "подтвердит")) else 0.0
        score += 8.0 if any(marker in lowered for marker in ("rsi", "adx", "vwap", "ema", "объём", "объем")) else 0.0
    ...
    def _engagement(text: str) -> float:
        ...
        score += 20.0 if re.search(r"\b(?:вход|цели|стоп-лосс)\b", text, re.IGNORECASE) else 0.0
    ...
    def _credibility(self, text: str) -> float:
        ...
        for claim in self.UNSUPPORTED_CLAIMS:
            if claim in lowered:
                score -= 35.0
        if not any(marker in lowered for marker in ("отмена сценария", "отмена:", "размер позиции", "допустимого риска", "без усреднения")):
            score -= 20.0
    ...
    def _spam_control(text: str) -> float:
        ...
        score = 100.0
        score -= max(0, hashtags - 2) * 18.0
        score -= max(0, emojis - 2) * 12.0
    ...
    def _length_score(self, text: str) -> float:
        ...
        if 380 <= size <= 780:
            return 100.0
        if self.MIN_LENGTH <= size < 380 or 780 < size <= self.MAX_LENGTH:
            return 82.0
        if 220 <= size <= max(self.MAX_LENGTH, 1000):
            return 60.0
        return 25.0
