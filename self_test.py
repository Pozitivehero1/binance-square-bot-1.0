...
def _test_side(side: str) -> None:
    ...
    with tempfile.TemporaryDirectory() as temp_directory:
        ...
        memory.add_post("TESTUSDT", text, post_style=drafts[0].style_id, signal_type=drafts[0].signal_type)
...
def _test_mistral_fact_lock() -> None:
    ...
    payload = {
        "candidates": [
            {
                "layout": "setup_first",
                "hook": "Импульс заметен, но спешить со входом рано",
                "interpretation": "Здесь важна реакция после движения, а не сама яркая свеча.",
                "question": "Вы бы ждали ретест или подтверждение без отката?",
                "fact_ids": ["setup", "volume", "mtf"],
            },
            {
                "layout": "levels_first",
                "hook": "План уже виден, теперь очередь за рынком",
                "interpretation": "Цена должна подтвердить идею, иначе наблюдение остаётся наблюдением.",
                "question": "Какое подтверждение для вас здесь главное?",
                "fact_ids": ["trend", "momentum"],
            },
            {
                "layout": "trigger_first",
                "hook": "Сделка начинается не с прогноза, а с условия",
                "interpretation": "Преимущество есть только пока структура не ломается.",
                "question": "Вы бы входили сразу или после повторной реакции?",
                "fact_ids": ["setup", "vwap", "changes"],
            },
            {
                "layout": "risk_first",
                "hook": "Хорошая картинка не отменяет красную линию",
                "interpretation": "Риск должен оставаться понятным ещё до открытия позиции.",
                "question": "Что заставило бы вас отказаться от такого входа?",
                "fact_ids": ["range", "volume", "mtf"],
            },
        ]
    }
    response = Mock()
    ...
    for draft in drafts:
        ...
        assert report.valid, report.reasons
    print("MISTRAL FACT LOCK: OK | one request | 4 valid candidates | ticker 1-3")
