...
def generate_chart(
    ...
    *,
    ...
    try:
        ...
        # Vary the analytical overlay with the editorial format.
        if visual_style == "level_map":
            price_axis.plot(frame.index, frame["VWAP"], color="#ff8c69", linewidth=1.25, linestyle="-.", label="VWAP")
        elif visual_style == "context_chart":
            price_axis.plot(frame.index, frame["EMA20"], color="#f5a623", linewidth=1.25, label="EMA20")
            price_axis.plot(frame.index, frame["EMA50"], color="#4a90e2", linewidth=1.2, label="EMA50")
            price_axis.plot(frame.index, frame["EMA200"], color="#9b59b6", linewidth=1.0, linestyle="--", label="EMA200")
            price_axis.plot(frame.index, frame["VWAP"], color="#ff8c69", linewidth=0.95, linestyle="-.", label="VWAP")
        else:
            price_axis.plot(frame.index, frame["EMA20"], color="#f5a623", linewidth=1.35, label="EMA20")
        ...
        # Keep the chart readable on mobile: lines stay on the plot, while labels
        # are collected into one compact plan panel instead of overlapping candles.
