...
MIN_FEED_APPEAL = float(os.getenv("MIN_FEED_APPEAL", "70"))
MIN_W2E_MARKET_SCORE = float(os.getenv("MIN_W2E_MARKET_SCORE", "58"))
MIN_CONVERSION_INTENT = float(os.getenv("MIN_CONVERSION_INTENT", "68"))
...
def _choose_market_candidate(
    ...
    eligible: List[Tuple[float, MultiTimeframeIndicators, SignalScore, Optional[float], str, AttentionSnapshot, MarketMonetizationSnapshot]] = []
    ...
    for mtf, score in ranked:
        ...
        # W2E candidate selection is intentionally conversion-oriented: technical
        # validity remains mandatory, but we strongly favour markets where traders
        # are already active and the setup is useful *now*.
        adjusted_score = (
            score.total * 0.30
            + attention.score * 0.27
            + monetization.score * 0.43
            - diversity_penalty
        )
        eligible.append((adjusted_score, mtf, score, funding, best_angle.id, attention, monetization))
        logger.info(
            "Candidate %s tech=%.1f attention=%.1f w2e=%.1f final=%.1f angle=%s "
            "15m=%+.2f%% vol=x%.2f extended=%s repeats=%s [%s]",
            mtf.symbol,
            score.total,
            attention.score,
            monetization.score,
    ...
    *,
    ...
    for index, draft in enumerate(drafts):
        try:
            ...
            if draft.content_format in {"hot_reaction", "one_problem", "crowd_trap", "chart_story"}:
                human_format_bonus += 5.0
            # Feed appeal is deliberately an independent ranking dimension.
            # Factual quality is still a hard validity gate below.
            adjusted_score = (
                report.score * 0.44 + appeal.score * 0.31 + conversion.score * 0.25
                - similarity_penalty - novelty_penalty
                + editorial_bonus + context_bonus + human_format_bonus
            )
            logger.info(
...
def _run_once() -> int:
    ...
    if monetization.score < MIN_W2E_MARKET_SCORE:
        logger.info(
            "Skip %s: W2E market score %.1f < %.1f (%s)",
            symbol, monetization.score, MIN_W2E_MARKET_SCORE, monetization.reason,
        )
        write_status(
            "skipped",
            "W2E market-conversion gate",
            symbol=symbol,
    ...
    try:
        if PUBLISH_IMAGES:
            ...
            if PUBLISH_MEDIA_MODE == "adaptive":
                human_chart_formats = {"hot_reaction", "one_problem", "crowd_trap", "chart_story", "level_story", "liquidity_map"}
