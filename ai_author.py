...
Не пиши как отчёт и не используй шаблоны. Пиши как реальный трейдер в соцсети: естественно, с личным наблюдением, с живым первым абзацем.
...
Структура:
1. сильный первый абзац с человеческим наблюдением, без заголовков
...
def validate(text, data):
    ...
    for v in [data["entry"], data["stop"]]:
        if str(round(float(v),2)) not in text:
            return False
    bad=["Матрица решения","Сверка контекста","ЗА","ПРОТИВ","Торговая схема","Параметры идеи","История движения"]
